...
// Update API configuration dynamically if needed
function setApiEndpoint(endpointType, url) {
  if (endpointType === 'BASE') {
    API_CONFIG.BASE_URL = url;
    API_CONFIG.ENDPOINTS = {
      VERIFY_PIN: url + '/verify_pin.php',
      GET_GAMES: url + '/get_games.php',
      LOGOUT: url + '/logout.php'
    };
  } else if (API_CONFIG.ENDPOINTS[endpointType]) {
    API_CONFIG.ENDPOINTS[endpointType] = url;
  }
}

// ================= EXTERNAL API CALLS =================

// Verify PIN against external PHP endpoint
async function verifyPin(pin) {
  try {
    ...
    // Use credentials: 'omit' since we're making cross-origin requests
    const response = await fetch(API_CONFIG.ENDPOINTS.VERIFY_PIN, {
      method: 'POST',
      body: formData,
    ...
    // Handle session management differently for external API
    if (result.success) {
      // Store session info locally since we can't rely on PHP sessions
      localStorage.setItem('externalSessionActive', 'true');
...
// Load games from external PHP endpoint
async function loadGames() {
  try {
    // Check if we have an active session
    const sessionActive = localStorage.getItem('externalSessionActive');
    ...
    if (sessionDuration > 24 * 60 * 60 * 1000) { // 24 hours in milliseconds
      localStorage.removeItem('externalSessionActive');
    ...
    const response = await fetch(API_CONFIG.ENDPOINTS.GET_GAMES, {
      method: 'GET',
      headers: {
    ...
    // If access denied, show PIN modal
...
async function handlePinAccess(pin) {
  ...
  if (result.success) {
    ...
    // Load games from external API
    await loadGames();
    
    // Remember session if checkbox checked
...
function openGameModal(game) {
  ...
  const optionalButtons = [
    ...
    { key: 'firmwareLink', text: 'Firmware', color: 'btn-blue' },
...
// ================= LOGOUT =================
function logout() {
  // Since we can't call PHP logout across domains, clear local storage
  localStorage.removeItem('externalSessionActive');
  localStorage.removeItem('sessionStartTime');
...
function checkRememberedSession() {
  ...
  if (sessionActive && sessionStart && remembered) {
    const now = Date.now();
    const sessionDuration = now - parseInt(sessionStart);
...
document.addEventListener('DOMContentLoaded', function() {
  console.log("🚀 Wolf Gaming Hub - Loading with External API Support...");
  
  // Check if user has remembered session
  const hasSession = checkRememberedSession();
  ...
  console.log("✅ System ready with External API Configuration");
});
...
// ================= EXPOSE API CONFIG FOR DYNAMIC UPDATES =================
window.setWolfGamingApiConfig = setApiEndpoint;
